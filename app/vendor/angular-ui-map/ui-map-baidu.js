...
(function() {
	...
	var app = angular.module('ui.map.baidu', ['ui.event']);
...
//			});
	...
	app.value('uiMapConfig', {}).directive('uiMap', ['uiMapConfig','$parse', function(uiMapConfig, $parse) {
			
			var mapEvents = 'click dblclick drag dragend ' 
							+ 'dragstart  mousemove mouseout ' 
							+ 'mouseover resize rightclick ';
			...
			return {
				...
				link: function(scope, elm, attrs) {
					...
					var map = new BMap.Map(elm[0], opts);
		...
		function(uiMapInfoWindowConfig, $parse, $compile) {
			...
			return {
				link: function(scope, elm, attrs) {
					...
					if (!infoWindow) {
						infoWindow = new window.BMap.InfoWindow(opts.content);  // 创建信息窗口对象
